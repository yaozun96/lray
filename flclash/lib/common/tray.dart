...
class Tray {
  ...
  Future<void> update({
    ...
    // 连接/断开选项
    final connectMenuItem = MenuItem(
      label: trayState.isStart ? '断开' : '连接',
      onClick: (_) async {
        globalState.appController.updateStart();
      },
    );
    menuItems.add(connectMenuItem);
    menuItems.add(MenuItem.separator());
    // 只显示规则和全局模式，隐藏直连
    for (final mode in Mode.values) {
      if (mode == Mode.direct) continue; // 隐藏直连
      // 自定义模式名称
      String modeName;
      if (mode == Mode.rule) {
        modeName = '智能模式';
        ...
        modeName = Intl.message(mode.name);
      }
      menuItems.add(
        MenuItem.checkbox(
          label: modeName,
